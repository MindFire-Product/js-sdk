 ...
 *
 * // 2. Load the script
 * <script src="https://mf-cdn.web.app/voice-chat-component-v100-stg.js" type="module"></script>
 *
 ...
 *   console.log('Consent accepted:', event.detail);
 *   // detail: {
 *   //   account_id, agent_id, sdk_version, consent_enabled,
 *   //   consent_message, accepted_at, page_url
 *   // }
 * });
 *
...
class VoiceChatComponent extends HTMLElement {
  constructor() {
    ...
    // This handles the case where data/history are set on the element
  ...
  _initializeState() {
    ...
    this.consentAcceptedForPendingStart = false;

    // Closed captions state
    this.ccEnabled = false;
    this.ccContainer = null;
    this._activeAgentCCLine = null;
    this._ccWordBuffer = [];
    this._ccDripTimer = null;
  }
  ...
  _bindMethods() {
    ...
    this._acceptConsent = this._acceptConsent.bind(this);
    this._cancelConsent = this._cancelConsent.bind(this);
  }
  ...
  /**
   * Emit conversation-ended event with transcript when available
   * @private
  ...
  _emitConversationEnded(trigger) {
    ...
    const now = Date.now();
    const durationMs =
      typeof this.conversationStartTime === "number"
        ? Math.max(0, now - this.conversationStartTime)
        : null;
    ...
    if (durationMs !== null) {
      ...
      detail.durationSeconds = durationMs / 1000;
    }

    this._emit("voice.conversation.ended", detail);
  ...
  /**
   * Append a transcript entry while maintaining ordered history
   * @private
   * @param {"agent" | "user"} speaker - Speaker identifier
   * @param {string} text - Transcript content
   */
  _appendTranscriptEntry(speaker, text) {
    if (!text) return;
    this.conversationTranscript.push({ speaker, text });
  }
  ...
  cleanup() {
    if (this.conversationActive || this.conversationTranscript.length > 0) {
      this._emitConversationEnded("component-disconnected");
    }
  ...
  /**
   * Set contextual data
   * @param {Object} value - Data object. Must be a non-null object.
   */
  set data(value) {
   ...
   * @returns {Object|Array|null} Current conversation history
   */
  get history() {
   ...
   * @param {Object|Array} value - Conversation history. Must be an object or array.
   */
  set history(value) {
   ...
   *
   * - Light mode modal is white (#ffffff). A very light primary colour would
   *   be invisible, so we blend it toward a dark neutral.
   * - Dark mode modal is #1f2937. A very dark primary colour would be
   *   invisible, so we blend it toward a light neutral.
   *
   * If the primary colour already has sufficient contrast we return it as-is.
   * Falls back to the original string if the colour cannot be parsed (e.g. named
   * colours or CSS variables).
   * @private
   */
  _getWaveColor(primaryColor) {
    ...
    if (!isDark && luminance > 0.5) {
      // Too light for a white modal — blend 50% toward dark neutral #374151
      return toRgb(blend(rgb, { r: 55, g: 65, b: 81 }, 0.5));
    }
    if (isDark && luminance < 0.15) {
      // Too dark for a dark modal — blend 50% toward light neutral #e5e7eb
  ...
  _requiresConsent() {
    return Boolean(
      this.agentConfig?.consent_enabled &&
      this.agentConfig?.consent_message?.trim()
    );
  }
  ...
  _showConsentModal() {
    ...
    const acceptButton = this.shadowRoot?.querySelector("#consent-accept-btn");
    acceptButton?.focus();
  }
  ...
  async _acceptConsent() {
    this._emitConsentAccepted();
    this.consentAcceptedForPendingStart = true;
  ...
  render() {
    ...
    const consentMessageHtml = this._escapeHtml(
      this.agentConfig.consent_message || ""
    ).replace(/\n/g, "<br>");
    ...
    this.shadowRoot.innerHTML = `
        ...
        : ""
                ...
                >
                    <div class="consent-modal">
                        <h3 class="consent-title" id="consent-title">Before you start</h3>
                        <p class="consent-message" id="consent-message">${consentMessageHtml}</p>
                        <div class="consent-actions">
                            <button class="consent-action secondary" type="button" id="consent-cancel-btn">Cancel</button>
                            <button class="consent-action primary" type="button" id="consent-accept-btn">Agree</button>
                        </div>
                    </div>
                </div>
                ` : ""}
        ...
        : ""
                          ...
                          <div class="agent-info">
                              <h3 class="agent-name" id="modal-agent-name">${this.agentConfig.name
      }</h3>
                        ...
                        >
                            <svg viewBox="0 0 24 24" fill="none" aria-hidden="true" xmlns="http://www.w3.org/2000/svg">
                                <path d="M12 3C10.34 3 9 4.34 9 6V11C9 12.66 10.34 14 12 14C13.66 14 15 12.66 15 11V6C15 4.34 13.66 3 12 3Z" fill="currentColor"/>
                                <path d="M5 11H7C7 13.76 9.24 16 12 16C14.76 16 17 13.76 17 11H19C19 14.53 16.39 17.44 13 17.93V21H11V17.93C7.61 17.44 5 14.53 5 11Z" fill="currentColor"/>
   ...
   */
    setupEventListeners() {
      ...
      { selector: "#mute-btn", event: "click", handler: this.toggleMute },
        ...
        {
          selector: "#consent-accept-btn",
          event: "click",
          handler: this._acceptConsent,
        },
        {
          selector: "#consent-accept-btn",
          event: "keydown",
        ...
        {
          selector: "#consent-cancel-btn",
          event: "click",
          handler: this._cancelConsent,
        },
        {
          selector: "#consent-cancel-btn",
          event: "keydown",
  ...
  handleKeyDown(e) {
    ...
    if (e.key === "Escape" && consentOverlay?.classList.contains("show")) {
      e.preventDefault();
      this._cancelConsent();
      return;
    }
  ...
  async getEphemeralKey() {
    try {
      ...
      const params = new URLSearchParams({ sdk_version: this.sdkVersion });
      const url = `${this.apiBaseUrl}/${this.apiVersion}/llms/token/${this.accountId}/${this.agentId}/${this.agentConfig.voice}?${params.toString()}`;
  ...
  toggleMute() {
    ...
    try {
      this._setMuteState(!this.isMuted);
  ...
  setMuted(isMuted) {
    if (this.isDestroyed) return;
    this._setMuteState(Boolean(isMuted));
  }

  _setMuteState(isMuted) {
  ...
  _updateMuteButton() {
    ...
    const muteIndicator = this.shadowRoot?.querySelector("#mute-indicator");
    ...
    muteIndicator?.classList.toggle("show", this.isMuted);
    muteIndicator?.setAttribute("aria-hidden", (!this.isMuted).toString());
  }
  ...
  /**
   * Append a complete caption line to the CC container
   * @private
  ...
  _appendCaption(speaker, text) {
    ...
    // If there's an active agent CC line being streamed, insert the user
    // caption BEFORE it — the user spoke first, their transcription just
    // arrived late (after the agent's deltas already started).
    if (this._activeAgentCCLine && this._activeAgentCCLine.parentNode === this.ccContainer) {
      this.ccContainer.insertBefore(line, this._activeAgentCCLine);
    } else {
  ...
  /**
   * Buffer agent caption delta text and drip-reveal words at spoken pace.
   * Creates a new CC line on first call per agent turn; subsequent calls
   * add words to the buffer which the drip timer reveals gradually.
   * @private
   * @param {string} speaker - Agent display name
   * @param {string} deltaText - Incremental transcript chunk
   */
  _streamAgentCaption(speaker, deltaText) {
    if (!this.ccContainer || !this.ccEnabled) return;

    // Create the CC line for this agent turn if it doesn't exist yet
    if (!this._activeAgentCCLine) {
      const line = document.createElement("div");
      line.style.cssText = "margin-bottom: 4px;";
      line.innerHTML = `<strong>${speaker}:</strong> <span class="cc-stream-text"></span>`;
      this.ccContainer.appendChild(line);
      this._activeAgentCCLine = line;
      this._ccWordCount = 0;
    }

    // Split delta into actual words only (whitespace excluded)
    const words = deltaText.split(/\s+/).filter(w => w.length > 0);
    this._ccWordBuffer.push(...words);
  }

  /**
   * Start the CC drip timer. Called when the agent's audio actually
   * begins playing so that text reveal is synced with audible speech.
   * @private
   */
  _startCCDrip() {
    if (this._ccDripTimer) return;
    // 300ms per word ≈ 200 wpm, close to natural conversational pace
    this._ccDripTimer = setInterval(() => {
      this._dripNextWord();
    }, 300);
  }

  /**
   * Reveal the next word from the buffer into the active CC line.
   * Stops the timer when the buffer is empty.
   * @private
   */
  _dripNextWord() {
    if (!this._activeAgentCCLine || this._ccWordBuffer.length === 0) {
      this._stopDripTimer();
      return;
    }

    const textSpan = this._activeAgentCCLine.querySelector(".cc-stream-text");
    if (textSpan) {
      const word = this._ccWordBuffer.shift();
      // Add a space before the word unless it's the first word or starts with punctuation
      const needsSpace = this._ccWordCount > 0 && !/^[.,!?;:'")\]\-—]/.test(word);
      textSpan.textContent += (needsSpace ? " " : "") + word;
      this._ccWordCount++;
    }
   ...
   */
  _stopDripTimer() {
    if (this._ccDripTimer) {
      clearInterval(this._ccDripTimer);
      this._ccDripTimer = null;
    }
  }

  /**
   * Finalize the current agent caption: flush any remaining buffered words
   * and reset state for the next agent turn.
   * @private
   */
  _finalizeAgentCaption() {
    // Flush remaining buffered words so nothing is lost
    if (this._activeAgentCCLine && this._ccWordBuffer.length > 0) {
      const textSpan = this._activeAgentCCLine.querySelector(".cc-stream-text");
      if (textSpan) {
        const remaining = this._ccWordBuffer.join(" ");
        textSpan.textContent += (this._ccWordCount > 0 ? " " : "") + remaining;
      }
      this._ccWordBuffer = [];
      if (this.ccContainer) {
        this.ccContainer.scrollTop = this.ccContainer.scrollHeight;
  ...
  async startConversation() {
    ...
    try {
      ...
      this.conversationActive = false;
        ...
        if (this._requiresConsent() && !this.consentAcceptedForPendingStart) {
      ...
      if (this.agentConfig.mcp_zapier) {
        ...
        const mcpZapierTool = tool({
          name: this.agentConfig.mcp_zapier.name,
          description: this.agentConfig.mcp_zapier.description,
          parameters: {
            type: "object",
            properties: {
              input: { type: "string" },
          ...
          execute: async (query) => {
            const url = `${this.apiBaseUrl}/${this.apiVersion}/llms/mcp/zapier/${this.accountId}/${this.agentId}`;
            ...
            const data = await response.json();
            console.log("Data from MCP Zapier:", data);
            return data;
          },
        });
        tools.push(mcpZapierTool);
      }

      if (
        this.agentConfig.has_file_search &&
        this.agentConfig.vector_store_id
      ) {
      ...
      // Build audio input config
      const audioInput = { turnDetection: turnDetection };
    ...
    this.session.transport.on("*", (event) => {
      ...
      switch (event.type) {
        ...
        case "output_audio_buffer.started":
          ...
          // Start dripping buffered CC words now that audio is playing
          this._startCCDrip();
          break;

        case "output_audio_buffer.stopped":
          this.toggleSoundWave(false);
          // Finalize the streaming agent caption line
          this._finalizeAgentCaption();
          break;
        ...
        case "response.output_audio_transcript.delta":
          // Stream agent caption progressively as deltas arrive
          if (event?.delta) {
            const agentFirstNameDelta = this.agentConfig?.name?.split(" ")[0] || "Agent";
            this._streamAgentCaption(agentFirstNameDelta, event.delta);
          }
          break;
        ...
        case "response.output_audio_transcript.done":
          ...
          if (event?.transcript) {
            this._appendTranscriptEntry("agent", event.transcript);
          }
          break;
        ...
        case "error":
          // When interrupt_response is false, the user speaking during an active
          // response triggers VAD → speech end → SDK tries to create a new response
          // → OpenAI rejects it because the previous response is still active.
          // This is expected behavior, not a fatal error — just ignore it.
          if (event.error?.code === "conversation_already_has_active_response") {
            ...
            // The SDK may have already interrupted audio playback, so reset waves
            this.toggleSoundWave(false);
            break;
          }
          ...
          this._emitConversationEnded("session-error");
          break;
      }
    });
  }
  ...
  _handleConversationError(error) {
    ...
    this._emitConversationEnded("conversation-error");
  }
  ...
  stopConversation() {
    try {
      ...
      // Remove CC container and reset CC button state
      this._stopDripTimer();
      this._removeCCContainer();
      this.ccEnabled = false;
      this._activeAgentCCLine = null;
